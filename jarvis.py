...
def say_jarvis_welcome() -> None:
    text = "Systems initialized. Welcome back, user. Diagnostic routines are fully active."
    try:
        log.info("Accessing native Windows SAPI subsystem...")
        speaker = win32com.client.Dispatch("SAPI.SpVoice")
        log.info("Streaming system welcome notification sequence...")
        speaker.Speak(text)
        log.info("Audio generation pipeline executed successfully.")
    except Exception as e:
        log.error("Core Engine Runtime Error: %s", e)
